...
def get_B(bin_centers1: torch.Tensor, bin_centers2: torch.Tensor, prior_B_x0_c: torch.Tensor,
          prior_B_ls: torch.Tensor, noise: float = 1e1*EPS, device: torch.device = 'cpu') -> torch.Tensor:
    """
    ...
    kernel = kernel + noise*torch.eye(n=kernel.shape[0], m=kernel.shape[1], device=device)

    return kernel


def get_B_factor(bin_centers1: torch.Tensor, bin_centers2: torch.Tensor, prior_B_x0_c: torch.Tensor,
                 prior_B_ls: torch.Tensor, device: torch.device = 'cpu'):
    """
    ...
    :param prior_B_ls: length-scale parameter of the RBF kernel used for the velocities
    :param device: device
    :return the factor of the bin matrix:
    """
    # B x B lower triangular matrix
    return torch.linalg.cholesky(get_B(bin_centers1, bin_centers2, prior_B_x0_c, prior_B_ls, device=device))
...
def get_D_factor(dim, device: torch.device = 'cpu'):
    """
    Computes the factor of the dimension matrix of shape D x D
    :param dim: dimension
    :param device: device
    :return the factor of the dimension matrix:
    """
    # D x D matrix
    return torch.eye(dim, device=device)


def get_R(dim, sigma, B_factor, C_factor, D_factor, device: torch.device = 'cpu'):
    """
    ...
    :param device: device
    :return: the capacitance matrix R:
    """

    R = torch.eye(dim, device=device) + \
        (1./sigma**2)*torch.kron(B_factor.T @ B_factor, torch.kron(C_factor.T @ C_factor, D_factor.T @ D_factor))

    return R


def get_R_factor(dim, sigma, B_factor, C_factor, D_factor, device: torch.device = 'cpu'):
    """
    ...
    :param D_factor: the dimension matrix of shape DxD
    :param device: device
    :return: the factor of the capacitance matrix R
    """

    R_factor = torch.linalg.cholesky(get_R(dim, sigma, B_factor, C_factor, D_factor, device=device))
